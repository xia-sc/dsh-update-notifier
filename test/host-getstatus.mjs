...
function createCtx() {
  ...
  const ctx = {
    logger: { info() {}, warn() {} },
    connection: {}, // no requestRejection → the loopback fence passes
    effect(fn) {
      const dispose = fn();
  ...
  return { ctx, routes, disposeAll: () => disposers.forEach((d) => d()) };
}
...
async function callRpc(routes, endpoint, method) {
  const route = routes.find((r) => r.path === "/dsh-update-rpc");
  assert.ok(route, "the plugin did not register /dsh-update-rpc");
  const envelope = { type: "client-request", rpcId: "test-1", method, payload: { args: {} } };
  const res = makeRes();
  const started = Date.now();
  await route.handler(makeReq(`/dsh-update-rpc/${endpoint}`, JSON.stringify(envelope)), res);
  const elapsed = Date.now() - started;
  assert.equal(res.response.status, 200, `unexpected HTTP ${res.response.status}`);
  const parsed = JSON.parse(res.response.body);
...
const { ctx, routes, disposeAll } = createCtx();
model.apply(ctx);
try {
  // wait until the plugin's startup timer has fired and its check is running
  await sleep(ASK_AT);
  const { value, elapsed } = await callRpc(routes, "getStatus", "getStatus");
