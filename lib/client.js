...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      "banner.sourceBoth": "GitHub 与 npm 一致，以 GitHub 为准",
      "banner.sourceGithub": "GitHub 已有新版本，npm 缓存滞后，以 GitHub 为准",
    ...
    var en = {
      ...
      "banner.cross": "npm: {latest} / {next} · GitHub: {github}",
      "banner.sourceBoth": "GitHub and npm agree — GitHub is source of truth",
      "banner.sourceGithub": "GitHub is ahead of npm cache — GitHub is source of truth",
    ...
    function createStore(ctx) {
      ...
      function applyResult(res) {
        ...
        var v = res.value || {};
        try {
          console.log("[dsh-update-notifier] query result", JSON.stringify({ current: v.current, latest: v.latest, next: v.next, github: v.github, hasUpdate: v.hasUpdate, target: v.target, channel: v.channel, source: v.source, checkedAt: v.checkedAt, error: v.error }));
          if (v.hasUpdate) console.log("[dsh-update-notifier] update available: " + v.current + " → " + v.target + " (" + v.channel + "/" + v.source + ")");
          else console.log("[dsh-update-notifier] up to date: " + v.current + " (latest:" + v.latest + " next:" + v.next + " github:" + v.github + ")");
        } catch {}
        emit({
          phase: "ready",
    ...
    function UpdateBanner(props) {
      ...
      }, [snap.current, snap.target, snap.hasUpdate]);

      // auto-hide up-to-date banner after 8s
      React.useEffect(function(){
        if (!snap.hasUpdate && snap.phase === "ready" && !snap.error && !dismissedUpToDate) {
          var id = setTimeout(function(){ setDismissedUpToDate(true); }, 8000);
          return function(){ clearTimeout(id); };
        }
      }, [snap.hasUpdate, snap.phase, snap.error, dismissedUpToDate]);

      if (snap.phase === "idle" || snap.phase === "loading") return null;

      // ── has update: dismissible per target version ──
      if (snap.hasUpdate) {
        if (snap.target && store.isDismissed(snap.target)) return null;
        ...
        var bodyText = t("banner.body", { current: snap.current || "?", target: snap.target || snap.github || snap.latest || "?", channel: channelLabel });
        var cmd;
        if (snap.channel === "github") {
          cmd = "npm i -g @deepseek-ai/dsh@" + (snap.target || snap.github || "latest");
        } else if (snap.channel === "next") {
          cmd = "npm i -g @deepseek-ai/dsh@next";
        } else {
          cmd = "npm i -g @deepseek-ai/dsh@latest";
        }
        ...
        function onCopy() {
          ...
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(function(){ setCopied(true); }).catch(function(){
        ...
        var crossText = t("banner.cross", { latest: snap.latest || "—", next: snap.next || "—", github: snap.github || "—" });
        var sourceNote = null;
        if (snap.source === "both") sourceNote = t("banner.sourceBoth");
        else if (snap.source === "github") sourceNote = t("banner.sourceGithub");
        else if (snap.source === "npm") sourceNote = t("banner.sourceNpm");

        var sourceStyle = snap.source === "github"
          ? { fontSize: 11, lineHeight: "16px", color: "var(--dsw-alias-state-warning-primary, #f59e0b)" }
          : S.meta;
        ...
        return h("div", { style: S.bannerWrap, "data-dsh-update": "banner" },
          ...
          h("div", { style: S.body },
            ...
            sourceNote ? h("div", { style: sourceStyle }, sourceNote) : null,
      ...
      if (snap.error) {
        return h("div", { style: Object.assign({}, S.bannerWrap, { borderColor: "var(--dsw-alias-state-warning-secondary, rgba(245,158,11,0.4))" }), "data-dsh-update": "banner-error" },
          ...
          h("div", { style: S.body },
            ...
            h("div", { style: S.meta }, t("banner.cross", { latest: snap.latest || "—", next: snap.next || "—", github: snap.github || "—" })),
      ...
      // ── up to date ──
      if (dismissedUpToDate) return null;
      var crossText2 = t("banner.cross", { latest: snap.latest || "—", next: snap.next || "—", github: snap.github || "—" });
      var sourceNote2 = null;
      if (snap.source === "both") sourceNote2 = t("banner.sourceBoth");
      else if (snap.source === "github") sourceNote2 = t("banner.sourceGithub");
      else if (snap.source === "npm") sourceNote2 = t("banner.sourceNpm");
      ...
      return h("div", { style: Object.assign({}, S.bannerWrap, { borderColor: "var(--dsw-alias-state-success-secondary, rgba(52,199,89,0.35))" }), "data-dsh-update": "banner-uptodate" },
        ...
        h("div", { style: S.body },
          h("div", { style: S.line }, "当前 " + (snap.current || "?") + " 已是最新" + (snap.github ? "（GitHub: " + snap.github + "）" : snap.latest ? "（npm: " + snap.latest + "）" : "")),
          h("div", { style: S.meta }, crossText2),
          sourceNote2 ? h("div", { style: S.meta }, sourceNote2) : null,
          h("div", { style: S.row },
            h("a", { href: "https://github.com/deepseek-ai/deepseek-harness/releases", target: "_blank", rel: "noreferrer", style: Object.assign({}, S.btnGhost, { textDecoration: "none", display: "inline-flex", alignItems: "center" }) }, t("banner.viewRelease")),
            h("a", { href: "https://github.com/deepseek-ai/deepseek-harness/tags", target: "_blank", rel: "noreferrer", style: Object.assign({}, S.btnGhost, { textDecoration: "none", display: "inline-flex", alignItems: "center" }) }, t("banner.viewTags")),
            h("span", { style: S.meta }, snap.checkedAt ? t("banner.checkedAt", { time: formatTime(snap.checkedAt) }) : "")
