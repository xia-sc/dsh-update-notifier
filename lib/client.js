  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dun-dock{display:inline-flex;align-items:center;gap:6px;max-width:280px;height:24px;padding:0 10px;border-radius:999px;" +
        ...
        "background:rgba(103,158,254,.12);background:color-mix(in srgb,var(--dsw-alias-state-business-primary,#679efe) 12%,transparent);" +
      ...
      ".dun-dock:hover{background:rgba(103,158,254,.2);background:color-mix(in srgb,var(--dsw-alias-state-business-primary,#679efe) 20%,transparent)}",
    ...
    // Endpoints are exact Fetch routes under the shared `/api` channel; the
    // `dsh-update-rpc/` segment keeps this plugin's namespace out of the API
    // Gateway's own endpoint space. The host registers one route per endpoint and
    // requires `envelope.method` to equal the route's last path segment.
    var RPC_CHANNEL = "/api";
    function rpc(ctx, endpoint, args) {
      return ctx.connection.rpc.call(RPC_CHANNEL, "dsh-update-rpc/" + endpoint, { args: args || {} });
    }
