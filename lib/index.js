...
function apply(ctx) {
  ...
  // ── RPC channel ──
  //
  // NEVER register this through `ctx.connection.rpc.handle(...)`: inside that
  // helper the route registers on the CONNECTION service's own fiber, whose
  // context cannot see `webServer` (connection only injects `credentials`), so
  // Cordis throws `cannot get property "webServer" without inject`, the loader
  // kills the whole plugin tree, and the Web GUI fails to boot. Mirror the
  // pattern used by dsh-opencode-go instead: mount the physical prefix route
  // DIRECTLY on `ctx.webServer.register(...)` from THIS fiber (which injects
  // `webServer`), wire the same `client-request` envelope contract, and reuse
  // `connection.requestRejection` for the trusted-host + browser-auth fence.
  const connectionService = ctx.connection;
  const rpcEndpoint = (pathname) => {
    const raw = pathname.slice("/dsh-update-rpc/".length);
    return raw !== "" && !raw.split("/").some((s) => s === "" || s === "." || s === ".." || !/^[A-Za-z0-9_$.-]+$/.test(s)) ? raw : "";
  };
  const rpcFetchHandler = {
    requestBodyMode: () => "buffered",
    fetch: async (request) => {
      const endpoint = rpcEndpoint(new URL(request.url).pathname);
      ...
      if (envelope.method !== endpoint) {
        return respond(rawId, {
          ok: false,
          error: { code: "gateway/bad-request", message: `method ${JSON.stringify(envelope.method)} does not match endpoint ${JSON.stringify(endpoint)}`, details: {} },
        });
      }
      try {
  ...
  ctx.effect(
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        let rejection;
        try {
          rejection = connectionService?.requestRejection?.(req);
        } catch {
          rejection = void 0; // connection service without a fence
        }
        if (rejection !== void 0) {
        ...
        // Buffered <-> fetch bridge, mirroring the /api transport shape.
        let body = "";
        try {
        ...
        const response = await rpcFetchHandler.fetch(request);
    ...
    "dsh-update-notifier: /dsh-update-rpc rpc channel"
  );
